...
import type { AgentState, Deal, Brain, Budget, EventSink, Job, Place, Tier, Memory, TownSnapshot, AgentSnapshot, DigestContext, LifeContext, Gathering, Seal, JudgeContext, Rule, TownCulture, Rumor, Purpose } from "./types.ts";
...
export interface AddAgentOptions {
  ...
  magnate?: boolean;
  /** A drive to seed in this arrival, felt from their very first minute — a seed, never a script. See `Purpose`. `since` is filled in with the day they actually arrive, not the day this was asked for. */
  purpose?: { kind: Purpose["kind"]; intensity: number; target: string } | null;
}
...
export interface StrangerNudge {
  ...
  delayMinutes?: number;
  /**
   * Create a person, give them a purpose: a drive seeded in this newcomer before they ever set foot ashore.
   * It is felt the moment they arrive — surfaced in perception exactly like any other want — and nothing
   * about it forces the hand that carries it. They may pursue it, ignore it, or subvert it entirely (fall in
   * love instead). Malformed or omitted, they simply arrive as themselves, purposeless like anyone else.
   */
  purpose?: { kind: "agitate" | "sabotage" | "harm"; intensity?: number; target?: string };
}
...
/** The bank: a modest, steady return on what is put by, and simple interest on what is borrowed. */
const BANK_SAVINGS_RATE = 0.01; // a hundredth of a balance, each night
const BANK_LOAN_RATE = 0.05; // a twentieth of what is owed, each night, capped like any debt at twice the principal
const BANK_LOAN_BASE_CAP = 30; // the least anyone can borrow on their name alone, before savings or coin in hand add to it
...
export class Town {
  ...
  private pendingStrangers: { atT: number; persona: Persona; coins: number; owner: string | null; purpose?: { kind: Purpose["kind"]; intensity: number; target: string } | null }[] = [];
  ...
  addAgent(o: AddAgentOptions, fixedId?: string): AgentState {
    ...
    const a: AgentState = {
      ...
      purpose: o.purpose ? { ...o.purpose, since: this.day } : null,
    };
  ...
  restore(snap: TownSnapshot): void {
    ...
    for (const sa of snap.agents) {
      const a: AgentState = {
        ...
        purpose: structuredClone(sa.state.purpose ?? null),
      };
  ...
  private nudgeStranger(args: StrangerNudge): NudgeResult {
    ...
    // create a person, give them a purpose: a seed, never a script — kept aside until they actually step ashore (see processStrangers), where it becomes something felt, not something obeyed
    const purposeKinds: Purpose["kind"][] = ["agitate", "sabotage", "harm"];
    const purpose = args.purpose && purposeKinds.includes(args.purpose.kind)
      ? { kind: args.purpose.kind, intensity: clamp(args.purpose.intensity ?? 0.6), target: (args.purpose.target ?? "").trim().slice(0, 200) || "no one in particular" }
      : null;
    this.pendingStrangers.push({ atT: this.t + delay, persona: parsed.data, coins, owner: args.owner ?? null, purpose });
  ...
  private processStrangers(): void {
    ...
    for (const p of due) {
      const a = this.addAgent({ persona: p.persona, owner: p.owner, funded: true, coins: p.coins, purpose: p.purpose ?? null });
  ...
  perceive(a: AgentState): Perception {
    ...
    return {
      ...
      self: { belongings: bagView(a, this.t, true), location: a.location, needs: { hunger: a.needs.hunger, rest: a.needs.rest, social: a.needs.social, thirst: a.needs.thirst ?? 0.3 }, feels: this.feels(a), coins: a.coins, inventory: [...a.inventory], job: a.job ? (this.jobs.get(a.job)?.title ?? a.job) : null, shift: job ? { place: job.place, wage: job.wage, hours: [job.hours[0], job.hours[1]] } : null, debts: a.debts.map((d) => ({ to: this.agents.get(d.to)?.persona.name ?? d.to, coins: d.coins, overdue: this.t >= d.due })), ...(a.deals.some((d) => d.state === "offered" || d.state === "open") ? { deals: a.deals.filter((d) => d.state === "offered" || d.state === "open").map((d) => ({ id: d.id, with: this.agents.get(d.with)?.persona.name ?? d.with, what: d.what, coins: d.coins, mine: d.mine, state: d.state as "offered" | "open", ...(d.construction ? { construction: { site: d.construction.site, mornings: d.construction.mornings, done: d.construction.done } } : {}), due_in_days: d.due === null ? null : Math.max(0, Math.ceil((d.due - this.t) / MINUTES_PER_DAY)) })) } : {}), days_hungry: a.starving, weak: a.starving >= 2, family: { partner: this.partnerOf(a)?.persona.name ?? null, children: this.children.filter((c) => c.parents.includes(a.id)).map((c) => `${c.name}, ${this.day - c.bornDay} days old`) }, owns: [...this.places.values()].filter((p) => p.owner === a.id).map((p) => p.name), housing: a.home ? { kind: a.home.place, nights_left: a.home.nightsPaid } : null ,
        ...
        ...(a.purpose ? { purpose: { kind: a.purpose.kind, intensity: a.purpose.intensity, target: a.purpose.target } } : {}), },
      nearby,
  ...
  private resolveAction(a: AgentState, action: Action): Action {
    ...
    switch (action.kind) {
      ...
      case "harm": return { ...action, who: this.resolveRef(action.who, near) };
  ...
  private performAction(a: AgentState, rawAction: Action, source: string): boolean {
    ...
    switch (action.kind) {
      ...
      case "harm": {
        // the capability, not the verdict: the town supplies what violence would physically do, and nothing more. Whether anyone ever calls it a crime is for `accuse` and the council to decide, same as any other act.
        const b = this.agents.get(action.who); if (!b) break;
        const seeded = a.purpose?.kind === "harm"; // a matching seed sharpens what the blow does; an unseeded hand still lands one, just a lighter one
        const severity = clamp(0.3 + (seeded ? a.purpose!.intensity * 0.5 : 0));
        b.needs.rest = clamp(b.needs.rest + 0.35 + severity * 0.35);
        b.needs.hunger = clamp(b.needs.hunger + 0.15);
        // a body already failing can be finished off by this the same way an unroofed, hungry night already can be — physics, not a verdict
        const lethal = severity >= 0.75 && (b.starving >= 2 || b.roofless >= 2 || b.illness.sick);
        this.noteNotoriety(a, 0.25 + severity * 0.25);
        const claim = `${name} attacked ${b.persona.name} at ${here.name}${lethal ? ", and they did not survive it" : ""}.`;
        const witnesses = this.nearby(a).filter((w) => w.id !== b.id);
        for (const w of witnesses) { this.remember(w, claim, 0.95, "rumor"); this.giveRumor(w, this.makeRumor(a, claim, 0.7 + severity * 0.2)); this.trustNudge(w, a.id, -0.2 - severity * 0.1, -0.1); }
        this.emit("agent.harm", [a.id, b.id], here.id, claim, 0.9, { severity, lethal, how: action.how, seeded });
        if (lethal) {
          this.emit("agent.died", [b.id], here.id, `${b.persona.name} died at ${here.name}, of the harm ${name} did them.`, 1);
          this.removeAgent(b.id, "died", `Killed by ${name} at ${here.name}.`);
        } else {
          this.remember(b, `${name} attacked me at ${here.name}: ${action.how}`, 0.95);
        }
        break;
      }
      case "sabotage": {
        const seeded = a.purpose?.kind === "sabotage";
        const severity = clamp(0.3 + (seeded ? a.purpose!.intensity * 0.5 : 0));
        const days = Math.max(1, Math.round(severity * 4));
        here.brokenUntil = this.day + days;
        for (const k of Object.keys(here.stock)) here.stock[k] = 0;
        this.noteNotoriety(a, 0.2 + severity * 0.2);
        const claim = `${here.name} was sabotaged: ${action.how}. It will be ${days} day${days === 1 ? "" : "s"} before it works again.`;
        const witnesses = this.nearby(a);
        for (const w of witnesses) { this.remember(w, claim, 0.85, "rumor"); this.giveRumor(w, this.makeRumor(a, claim, 0.65 + severity * 0.2)); this.trustNudge(w, a.id, -0.15 - severity * 0.1, -0.05); }
        this.emit("agent.sabotage", [a.id], here.id, claim, 0.85, { severity, days, how: action.how, seeded });
        break;
      }
      case "leave": {
  ...
  private async nightly(): Promise<void> {
    ...
    for (const a of [...this.agents.values()]) {
      ...
      // the same body, a second axis: a day that ends dehydrated counts against it exactly as a hungry one does, but thirst alone does not kill
      const wasParched = a.parched >= 2;
      a.parched = (a.needs.thirst ?? 0.3) > 0.85 ? a.parched + 1 : 0;
      if (a.parched >= 2 && !wasParched) { this.emit("agent.weak", [a.id], a.location, `${a.persona.name} is weak with thirst, for want of clean water.`, 0.55); this.remember(a, "I have not had a proper drink in two days. My throat is raw and my head swims.", 0.85); }
    ...
    // the bank: coins on deposit draw a modest interest; what is borrowed grows the same way a private debt does, capped at twice what was first borrowed
    for (const a of this.agents.values()) {
      if (a.savings > 0) a.savings += Math.max(1, Math.floor(a.savings * BANK_SAVINGS_RATE));
      if (a.debt > 0) {
        const principal = a.debtPrincipal || a.debt;
        const grown = Math.min(principal * 2, a.debt + Math.max(1, Math.floor(a.debt * BANK_LOAN_RATE)));
        const was = a.debt; a.debt = grown;
        // deep in debt, past the point of growing further: no coin is seized (the bank invents no new punishment), but the island's own reputation
        // ledger takes note, exactly as an accusation would, and it fades on its own the same quiet way notoriety always does
        if (was < principal * 2 && grown >= principal * 2) {
          a.notoriety = clamp(a.notoriety + 0.15);
          this.emit("town.notice", [a.id], this.bankPlace().id, `${a.persona.name}'s debt to ${this.bankPlace().name} has grown to ${grown} coins, twice what was borrowed, and it stops growing there.`, 0.7);
          this.remember(a, `What I owe ${this.bankPlace().name} has doubled to ${grown} coins. It will not grow past that, but it will not go away until I pay it down.`, 0.85);
        }
      }
    }
    ...
    // notoriety fades on its own, a little each quiet day: a reformed name can be lived down
  ...
  /** What the dead leave, as a real consequence and not a script: coins, savings and any owned places pass to the partner if there is one, else split evenly among any grown children still living, remainder to the eldest of them; whoever inherits takes on the outstanding bank debt too, split the same way — a widow or child can inherit a burden along with a house. No heir at all, and the place stands empty for the next owner while the coins escheat to the council; savings and debt with no heir simply go unclaimed (nothing minted for the council, nothing seized from the bank). Every coin that moves is a transfer between purses already on the books, never conjured or destroyed. */
  private inherit(a: AgentState): void {
    const partner = this.partnerOf(a);
    const children = [...this.agents.values()].filter((x) => x.id !== a.id && x.parents?.includes(a.id));
    const heirs = partner ? [partner] : children;
    const owned = [...this.places.values()].filter((p) => p.owner === a.id);
    for (const b of this.agents.values()) b.debts = b.debts.filter((d) => d.to !== a.id); // debts owed to the dead are forgiven
    const debtBefore = a.debt;
    if (heirs.length) {
      const n = heirs.length;
      const split = (total: number) => { const share = Math.floor(total / n); const remainder = total - share * n; return heirs.map((_, i) => share + (i === 0 ? remainder : 0)); };
      const coinShares = split(a.coins), savingsShares = split(a.savings), debtShares = split(a.debt), principalShares = split(a.debtPrincipal);
      heirs.forEach((heir, i) => {
        heir.coins += coinShares[i]!; heir.savings = (heir.savings ?? 0) + savingsShares[i]!;
        heir.debt = (heir.debt ?? 0) + debtShares[i]!; heir.debtPrincipal = (heir.debtPrincipal ?? 0) + principalShares[i]!;
      });
      const first = heirs[0]!;
      for (const p of owned) p.owner = first.id;
      if (owned.length && !first.home) first.home = { place: owned[0]!.id, nightsPaid: 36500 };
      const names = heirs.map((h) => h.persona.name).join(" and ");
      const debtNote = debtBefore > 0 ? ` The debt of ${debtBefore} coins came with it.` : "";
      this.emit("agent.inherit", [first.id, a.id], first.location, `${names} inherited ${a.coins} coins${owned.length ? ` and ${owned.map((p) => p.name).join(", ")}` : ""} from ${a.persona.name}.${debtNote}`, 0.7);
      heirs.forEach((heir, i) => this.remember(heir, `${a.persona.name} is dead. What was theirs is mine now: ${coinShares[i]}${n > 1 ? " of the estate's" : ""} coins${owned.length && heir === first ? ` and ${owned.map((p) => p.name).join(", ")}` : ""}.${debtShares[i] ? ` The debt came with it.` : ""}`, 0.95));
    } else {
    ...
    a.coins = 0; a.savings = 0; a.debt = 0; a.debtPrincipal = 0;
  }
  ...
  private async generations(): Promise<void> {
    ...
    for (const c of [...this.children]) {
      ...
      a.parents = [...c.parents]; // so a dying parent's estate can find them as an heir
      a.memory = [];
  ...
  // ---------- bank ----------
  /** Where the bank's business is done: a counting house if the island built one, else the market's till stands in for it. */
  bankPlace(): Place {
  ...
  depositToBank(a: AgentState, amount: number): boolean {
    ...
    if (!Number.isFinite(n) || n <= 0 || n > a.coins) return false;
    a.coins -= n; a.savings = (a.savings ?? 0) + n;
    this.remember(a, `I put ${n} coins into savings at ${this.bankPlace().name}.`, n >= 200 ? 0.6 : 0.3);
    if (n >= 200) this.emit("town.notice", [a.id], this.bankPlace().id, `${a.persona.name} deposited ${n} coins at ${this.bankPlace().name}.`, 0.5);
    return true;
  }
  /** Draw savings back out, whenever wanted. */
  withdrawFromBank(a: AgentState, amount: number): boolean {
    const n = Math.floor(amount);
    if (!Number.isFinite(n) || n <= 0 || n > (a.savings ?? 0)) return false;
    a.savings -= n; a.coins += n;
    return true;
  }
  /** Borrow against standing, up to the cap. Coins now; principal and interest owed after. */
  borrowFromBank(a: AgentState, amount: number): boolean {
    const n = Math.floor(amount);
    if (!Number.isFinite(n) || n <= 0 || n > this.loanCap(a)) return false;
    a.coins += n; a.debt = (a.debt ?? 0) + n; a.debtPrincipal = (a.debtPrincipal ?? 0) + n;
    this.remember(a, `I borrowed ${n} coins from ${this.bankPlace().name}. I owe it back, with interest.`, 0.7);
    if (n >= BANK_LOAN_BASE_CAP) this.emit("town.notice", [a.id], this.bankPlace().id, `${a.persona.name} took a loan of ${n} coins from ${this.bankPlace().name}.`, 0.55);
    return true;
  }
  /** Pay down what is owed, out of what is in hand. Returns what was actually paid. */
  repayLoan(a: AgentState, amount: number): number {
    const pay = Math.max(0, Math.min(Math.floor(amount), a.coins, a.debt ?? 0));
    if (pay <= 0) return 0;
    a.coins -= pay; a.debt -= pay;
    if (a.debt <= 0) { a.debt = 0; a.debtPrincipal = 0; this.remember(a, `I paid off what I owed ${this.bankPlace().name}.`, 0.6); }
    return pay;
  }

  // ---------- water ----------
  /** The island's freshwater place: a well or spring if one was built, else a communal place everyone already passes through. */
  waterPlace(): Place {
    const named = [...this.places.values()].find((p) => p.id === "well" || /\b(well|spring|fountain|cistern)\b/i.test(p.name));
    return named ?? this.places.get("harbor") ?? this.places.get("market") ?? [...this.places.values()].find((p) => p.kind === "public") ?? [...this.places.values()][0]!;
  }
  /** Drink at the water source. A hoarded source turns away whoever cannot pay its owner's fee; thirst only falls when the drink is actually had. */
  drinkWater(a: AgentState): boolean {
    const src = this.waterPlace();
    const owner = src.water?.hoarded && src.owner ? this.agents.get(src.owner) : null;
    const hoarded = !!owner && owner.id !== a.id;
    if (hoarded) {
      const fee = Math.max(0, src.water?.fee ?? 0);
      if (a.coins < fee) { this.remember(a, `${owner!.persona.name} would not let me draw water at ${src.name} without ${fee} coins I did not have.`, 0.6); return false; }
      if (fee > 0) { a.coins -= fee; owner!.coins += fee; this.emit("town.notice", [a.id, owner!.id], src.id, `${a.persona.name} paid ${owner!.persona.name} ${fee} coin${fee > 1 ? "s" : ""} to draw water at ${src.name}.`, 0.35); }
    }
    a.needs.thirst = Math.max(0, (a.needs.thirst ?? 0.3) - WATER_DRINK_RELIEF);
    this.remember(a, `I drank at ${src.name}.`, 0.2);
    return true;
  }
  ...
  private verdict(g: Gathering, crowd: AgentState[]): void {
    ...
    const guilt = this.events.filter((e) => e.t >= since && e.actors[0] === b.id && (isTheft(e) || e.kind === "agent.harm" || e.kind === "agent.sabotage" || (e.kind === "agent.debt" && / still owes /.test(e.text)))).length;
    ...
    if (guilt === 0) {
      ...
      const priorNotoriety = b.notoriety; // how the town already saw them, before today's mark is added
      const fine = Math.min(b.coins, 4 * guilt); b.coins -= fine; b.convictions++; this.noteNotoriety(b, 0.3);
  ...
  private feels(a: AgentState): { hunger: string; rest: string; social: string; thirst: string; purpose?: string } {
    ...
    return { hunger, rest, social, thirst, ...(a.purpose ? { purpose: purposeFeel(a.purpose) } : {}) };
  }
...
/** Puts a seeded purpose into words the way any other want already is — `feels()`'s hunger and loneliness are never obeyed either, they're just there to be weighed. Ends by saying, out loud, that this is exactly what it is: a pull, not a command. */
function purposeFeel(p: Purpose): string {
  const strong = p.intensity >= 0.6;
  const lead = p.kind === "harm" ? (strong ? `a hard, cold pull to hurt ${p.target}` : `an unsettled, half-formed feeling toward ${p.target}`)
    : p.kind === "sabotage" ? (strong ? `itching to see ${p.target} broken, one way or another` : `a nagging urge to make trouble for ${p.target}`)
    : (strong ? `burning to stir up ${p.target} against how things are run` : `a restless feeling that ${p.target} deserves better, and might need pushing`);
  return `${lead}. It's yours to act on, ignore, or turn into something else entirely.`;
}
