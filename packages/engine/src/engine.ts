...
export class Town {
  ...
  private verdict(g: Gathering, crowd: AgentState[]): void {
    ...
    if (guilt === 0) {
      ...
      { const claim = `The council fined ${b.persona.name} ${fine} coins for "${g.note}".`; for (const w of witnesses) { this.nudge(w, b.id, -0.1 - priorNotoriety * 0.1, -0.05 - priorNotoriety * 0.05); this.remember(w, claim, 0.6, "rumor"); this.giveRumor(w, this.makeRumor(b, claim, 0.5 + priorNotoriety * 0.2)); } }
      ...
      for (const knower of group) {
        ...
        for (const r of [...knower.gossip]) {
          for (const other of group) {
            ...
            if (this.agents.has(r.about) && r.about !== other.id) this.nudge(other, r.about, -0.04 - passedRumor.strength * 0.12, -0.02);
