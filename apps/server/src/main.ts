...
import { Town, Rng, MINUTES_PER_DAY, sha256, canonicalEvent, PACKS as WORLD_PACKS, ISLAND } from "@unwatched/engine";
...
const placeView = (p: import("@unwatched/engine").Place) => ({ storedCount: (p.keptStorage ?? []).reduce((n,b)=>n+b.items.length,0) + [...town.agents.values()].reduce((n,a)=>n+(a.storage?.find(s=>s.place===p.id)?.items.length??0),0), looseCount: p.looseItems?.length ?? 0, decorations: p.decorations ?? [], community: publicProject(town, p), id: p.id, hasHistory: !!p.history, name: p.nickname ? `${p.name} (${p.nickname})` : p.name, kind: p.kind, exits: p.exits, crowd: town.crowd(p.id), x: p.x, y: p.y, district: p.district, sprite: p.sprite, landUse: p.landUse, ...(p.look ? { look: p.look } : {}), ...(Object.keys(p.stock).length ? { stock: p.stock } : {}), owner: p.owner ? (town.agents.get(p.owner)?.persona.name ?? null) : null, site: p.site ? { what: p.site.what, name: p.site.name, by: town.agents.get(p.site.by)?.persona.name ?? p.site.by, done: p.site.labor, of: p.site.laborNeeded } : null, beds: p.beds ? { price: p.beds.price, free: p.freeBeds ?? 0 } : null });
...
// ---- The provocateur trail (§14): the hub only ingests and aggregates what an island chooses to report; it never
// computes guilt itself. This island reports what IT perceived — a name's notoriety, and whether they are a known
// fugitive — whenever something worth noting about that name happens here, so trouble follows a person island to
// island the same way their notoriety and fugitive status already ride the boat in-process (see `arrive` in the engine).
const reportPerson = (agentId: string, notoriety: number, fugitive: boolean, note: string, alias: string) =>
  hubPost(`/people/${encodeURIComponent(agentId)}/report`, { island: TOWN_ID, notoriety, fugitive, note, alias });
/** Called for every event the town emits; a no-op unless a hub is configured and the event actually says something about a notorious or fugitive name. */
function reportProvocateurTrail(e: { kind: string; actors: string[]; text: string }): void {
  if (!HUB_URL) return;
  if (!["agent.harm", "agent.sabotage", "town.verdict", "town.gathering", "town.notice", "agent.leave"].includes(e.kind)) return;
  for (const id of e.actors) {
    const a = town.agents.get(id);
    if (a && (a.notoriety > 0 || a.fugitive)) void reportPerson(a.id, a.notoriety, !!a.fugitive, e.text, a.persona.name);
  }
}
if (HUB_URL) {
